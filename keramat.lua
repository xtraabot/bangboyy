...
-- variabel yang dipakai di patch
local selectedFish = nil
local btn1 -- nanti diisi setelah tombol dibuat

-- override require untuk patch modul
require = function(module)
    local result = oldRequire(module)
    if module == FishingConfigModule then
        -- patch fungsi di modul
        local oldSelect = result.selectFish
        result.selectFish = function(...)
        ...
        result.GetRarityWithPity = function(...)
            if (btn1 and btn1.BackgroundColor3 == Color3.fromRGB(0,200,0)) or selectedFish then
                return "Secret"
            end
            return oldRarity(...)
        end
...
-- alias dan daftar nama ikan sudah ada di atas
...
local function chooseFish(fishName)
    ...
    -- set probability ikan pilihan + pity
    for _, fish in pairs(FishingConfig.FishTable) do
...
btn3.MouseButton1Click:Connect(function()
    -- tampilkan submenu di kanan menu utama
