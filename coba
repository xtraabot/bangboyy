...
local function forceReloadConfig()
    ...
    -- pasang hook sekali di sini (aman, tidak loop terus-menerus)
    if env and env.module_upvr_11 then
        ...
        if not getgenv()._rarityHooked or getgenv()._rarityHooked ~= currentRarity then
            ...
            hookfunction(currentRarity, function(...)
                local rarity = currentRarity(...)

                -- Secret ON → tambah peluang, bukan override total
                if (btn1 and btn1.BackgroundColor3 == Color3.fromRGB(0,200,0)) then
                    if rarity ~= "Secret" and math.random() < 0.001 then
                        rarity = "Secret"
                    end
                    -- AUTO-OFF: matikan booster setelah Secret keluar
                    if rarity == "Secret" then
                        btn1.BackgroundColor3 = Color3.fromRGB(170,0,0) -- OFF
                ...
                -- Limited ON → tambah peluang, bukan override total
                if (btn3 and btn3.BackgroundColor3 == Color3.fromRGB(0,200,0)) then
                    if rarity ~= "Limited" and math.random() < 0.001 then
                        rarity = "Limited"
                    end
                    -- AUTO-OFF: matikan booster setelah Limited keluar
                    if rarity == "Limited" then
                        btn3.BackgroundColor3 = Color3.fromRGB(170,0,0) -- OFF
                ...
                return rarity
            end) -- tutup hookfunction
        end -- tutup if rarityHooked
    end -- tutup if env
end -- tutup forceReloadConfig
...
title1.Text = "LIMITED EDITION v1.4 Test"
title1.TextSize = 8
...
local btn3 = createButton("Limited ON",1,2)
local btn4 = createButton("Limited OFF",2,2)
...
-- patch require untuk FishingConfigModule
require = function(module)
    local result = oldRequire(module)
    if module == FishingConfigModule then
        -- Patch GetRarityWithPity saja (booster Secret & Limited)
        local oldRarity = result.GetRarityWithPity
        result.GetRarityWithPity = function(...)
            local rarity = oldRarity(...)
...
-- baru require modul setelah patch
...
btn1.MouseButton1Click:Connect(function()
    ...
    -- ubah semua ikan Secret/Limited jadi probability lebih tinggi tapi realistis
    for _, fish in pairs(FishingConfig.FishTable) do
        if fish.rarity == "Secret" then
            fish.probability = math.max(fish.probability, 0.001) -- aman, tidak ekstrem
        end
    end

    -- atur pengaturan pity & rarity weights untuk Secret
    FishingConfig.Pity.Secret.maxPity = 1000 -- default 2000
    FishingConfig.Pity.Secret.baseBoost = 0.6 -- default 0.3
    FishingConfig.Pity.Secret.maxMultiplier = 3.2 -- default 1.6
    FishingConfig.RarityWeights.Secret = 2 -- default 0.2
...
btn3.MouseButton1Click:Connect(function()
    ...
    -- ubah semua ikan Limited jadi probability lebih tinggi tapi realistis
    for _, fish in pairs(FishingConfig.FishTable) do
        if fish.rarity == "Limited" then
            fish.probability = math.max(fish.probability, 0.001) -- aman, tidak ekstrem
        end
    end

    -- atur pengaturan pity & rarity weights untuk Limited
    FishingConfig.Pity.Limited.maxPity = 1500 -- default 3000
    FishingConfig.Pity.Limited.baseBoost = 0.8 -- default 0.4
    FishingConfig.Pity.Limited.maxMultiplier = 3.7 -- default 1.9
    FishingConfig.RarityWeights.Limited = 0.001 -- default 0.001
...
btn4.MouseButton1Click:Connect(function()
    ...
    -- restore pengaturan pity & rarity weights Limited
    FishingConfig.Pity.Limited.maxPity = 3000
    FishingConfig.Pity.Limited.baseBoost = 0.4
    FishingConfig.Pity.Limited.maxMultiplier = 1.9
    FishingConfig.RarityWeights.Limited = 0.001
...
btn5.MouseButton1Click:Connect(function()
    ...
    for rodName, _ in pairs(FishingConfig.MinigameSettings.rodTapCount) do
        FishingConfig.MinigameSettings.rodTapCount[rodName] = 3
    end
    FishingConfig.MinigameSettings.rodTapCount.default = 3
